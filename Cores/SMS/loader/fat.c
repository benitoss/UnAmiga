...
int fat_init()
{
	...
	if (!sd_load_sector(data_buffer, sector)) {
		printf("Error loading Sector 0\n");
		return FALSE;
	}

	if ((data_buffer[0x1fe]!=0x55) || (data_buffer[0x1ff]!=0xaa)) {
		printf("Wrong Sector 0\n");
		return FALSE;
	}
	...
	default:
		if (data_buffer[0x55]==0x33) { //check possible FAT type when NO MBR found (32)
			...
			printf("Unsupported FileSystem (FAT16/32 only)\n");
			return FALSE;
		}
	}
	...
	if (!sd_load_sector(data_buffer, sector)) {
		printf("Error while loading boot sector\n");
		return FALSE;
	}

	if ((data_buffer[0x1fe]!=0x55) || (data_buffer[0x1ff]!=0xaa)) {
		printf("Wrong boot record\n");
		//console_print_byte(data_buffer[0x1fe]); //q
		//console_print_byte(data_buffer[0x1ff]); //q
	...
	if ((data_buffer[11]!=0) || (data_buffer[12]!=2)) {
		printf("sector size != 0x200\n");
		printf("%02x", data_buffer[11]);
		printf("%02x", data_buffer[12]);
		return FALSE;
	}
